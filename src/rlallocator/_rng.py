...
All stochastic code (synthetic panel generation, the across-seed bootstrap) must
draw from a generator produced here, never from the global ``numpy.random`` state,
so that a single master seed reproduces an entire run. We use the PCG64 bit
generator (numpy's default, statistically robust) and derive independent
substreams via :meth:`numpy.random.SeedSequence.spawn`.
...
# quantcore-candidate: mirrors rl-trader:src/rltrader/_rng.py
...
def spawn_substreams(seed: int, n: int) -> list[np.random.Generator]:
    ...
    always yields the same children. Used to give each training seed an
    independent draw of the synthetic panel / training noise.
