"""Real Polygon.io EOD data provider (the optional offline cross-asset path).
...
and assembles a wide ``date x ticker`` panel of adjusted closes (inner-joined across
tickers, so only dates present for *every* requested ticker survive).
...
- ``import httpx`` is LAZY (inside :meth:`PolygonProvider.fetch`); a urllib fallback
  is used when httpx is absent, so the ``data`` extra is optional and importing this
  module has no side effects.
- HTTP ``429 Too Many Requests`` is retried with exponential backoff.
- The deployed default does NOT require a key — this is the OPTIONAL offline CLI path;
  callers (see :func:`rlallocator.data.loaders.load_multi_asset_panel`) fall through
  to the synthetic panel on any failure.
"""
...
def _load_api_key_from_dotenv() -> str | None:
    ...
    (ignoring blanks, comments, and an optional ``export`` prefix). Returns the value
    if found, else ``None``. Never raises on a malformed file.
    """
...
class PolygonProvider:
    ...
    api_key:
        ...
        ``POLYGON_API_KEY`` environment variable, then from a ``.env`` file in the
        repo root.
    max_retries:
        How many times to retry a single request after an HTTP ``429`` before giving
        up.
    backoff_base:
        Base seconds for exponential backoff between ``429`` retries (attempt ``i``
        sleeps ``backoff_base * 2**i``).
    timeout:
    ...
    def fetch(self, tickers: list[str], start: date, end: date) -> pd.DataFrame:
        ...
        LAZY IMPORT: ``httpx`` (the ``data`` extra) is imported inside this method;
        when it is absent a stdlib ``urllib`` fallback is used. Either way, importing
        this module touches no network.

        Parameters
        ----------
        tickers:
            Asset symbols to fetch (e.g. ``["SPY", "TLT", "GLD"]``).
        start, end:
        ...
        -------
        pandas.DataFrame
            Adjusted closes with rows = date and columns = ticker (in request order),
            inner-joined so every row has a value for every ticker.
    ...
    def _get_json(self, ticker: str, start: date, end: date) -> dict[str, Any]:
        ...
        Tries ``httpx`` first (lazy import) and falls back to ``urllib`` when it is
        unavailable. Raises on a non-retryable HTTP error or after exhausting the
        ``429`` retry budget.
        """
    ...
    def _series_from_payload(payload: dict[str, Any], ticker: str) -> pd.Series:
        ...
        Polygon returns ``results`` as a list of bar dicts with ``t`` (epoch ms, UTC)
        and ``c`` (adjusted close; ``adjusted=true`` was requested).
        """
